...
<!-- DK: Show the code for fitting and printing models, and for any other code chunks that are small/important. DONE-->
...
<!-- DK: Perhaps add summary paragraph done. 
Mention the questions in the introduction. -- Done in the wisdom section
Then sentence or two summary about Cardinal Virtues. done -->
...
<!-- DK: Start with less precise version of the question, and then refine it. DONE-->
...
First, do we need a **causal** or **predictive** model? In our question above we have a predictive model as we only have one outcome which is a gubernatorial candidate's longevity.

Second, what is the outcome? A person's longevity after the election will be the **outcome**. The questions, however, leave unclear details. For example, what time period does the Preceptor Table cover? In order to dive deeper into these questions we need to engage in a conversation with the person that is asking us these questions.

<!-- DK: Pick units, either all candidates from 2000 through 2023, or all candidates in the 2024 election. Using 2000 through 2023.-->

Third, what are the **units**? Our units for this scenario would be gubernatorial candidates because the questions are about the life expectancy of unique candidates. Here, we want to discuss all gubernatorial candidates from 2000 through 2023 in the United States. However, another question arises: are we looking for the average of all gubernatorial candidates? Are we looking specifically into the average of all female gubernatorial candidates? Or male gubernatorial candidates? In our situation, it is important to refine the question because that allows us to accurately respond to the question at hand.
...
**How many years would we expect two gubernatorial candidates --- one male and one female, both 10 years older than the average candidate --- to live after any elections from 2000 to 2023 in the United States? How different will their lifespans be?*
...
We would have rows for *every* gubernatorial election candidate in U.S. history. We may want further details, such as election year. 
...
```{r}
#| code-fold: false
pp_check(fit_1, plotfun = "hist", nreps = 3, binwidth = 5)
```

Our graph in the darker blue represents our actual data. As we can see with the lighter blue graph, our fitted model is able to generate a distribution that is similar when compared to the actual data. One important item that we want to note is that for the actual data there is no value that is under 0 or over 60 for longevity of a candidate, however, in the fitted model we are able to see several general values for which the longevity of a candidate is under 0 or over 60. For the most part our fitted model does a great job in generating a distribution through the "fake-data" simulation when compared to the actual data set.
...
#| code-fold: false
#| warning: false
pp_check(fit_2, plotfun = "hist", nreps = 3, binwidth = 10)
```

With the lighter blue graph, our fitted model is able to generate a distribution that is reasonably similar to the actual data. However, the "fake-data" produces some values for longevity which are *impossible.* We know that the longevity of a person can not decrease below 0. But, with our "fake-data" simulations, we see several values that are less than 0. This is a flaw in our model. Is it a serious flaw? That is tough to decide. For the most part though our fitted model does a great job in generating a distribution through the "fake-data" simulation when compared to the actual data set.
...
```{r}
#| code-fold: false
pp_check(fit_3, plotfun = "hist", nreps = 3, binwidth = 0.1)
```

Our fitted model generates a distribution that is very similar to the distribution of values from our actual data.
...
For the most part the model is really accurate as we are able to see the distribution is very similar to the actual data proving it to be reasonably accurate. 
...
Looking at our posterior probability distributions above, we can see that male candidates are expected to live longer. But how much longer? Does the initial age affect the posterior for expected years after the candidate's election? Based on our previous knowledge we know that females tend to live longer than the males which is surprising. With our graph we have a higher tendency of the male candidates living longer simply because of the amount of male candidates that run for election. As in previous chapters, we can manipulate distributions in, more or less, the same way that we manipulate simple numbers. If we want to know the difference between two posterior distributions, we can simply subtract. 
...
Looking at our results, you can see that our intercept value is around `r round(coef(fit_5)["(Intercept)"], 0)`. The average electoral candidate that won, who had been alive the average number of years of all candidates, would live another 2 years or so after the election. 
...
```{r}
#| code-fold: false
pp_check(fit_5, plotfun = "scatter", nreps = 3)
```
